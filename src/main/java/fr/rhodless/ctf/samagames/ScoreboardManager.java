...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = Main.getMain().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Main.getMain().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);
 
        reloadingTask = Main.getMain().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Main.getMain().getExecutorMonoThread().execute(scoreboard::reloadData);
    ...
    private String colorIpAt() {
        ...
        if (ipCharIndex > 0) {
            ...
            formattedIp.append(ChatColor.GRAY).append(ip.substring(ipCharIndex - 1, ipCharIndex));
        } else {
